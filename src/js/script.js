...
async function loadDashboard() {
  ...
  const data = await apiFetch(`api/get_dashboard_data.php?stagione=${state.stagione}`);
  if (!data) return;
...
async function loadCosti() {
  ...
  const res = await apiFetch(`api/get_costi_list.php?stagione=${state.stagione}`);
...
async function loadProduzione() {
  ...
  const res = await apiFetch(`api/get_produzione_list.php?stagione=${state.stagione}`);
...
async function loadVendite() {
  ...
  const res = await apiFetch(`api/get_vendite_list.php?stagione=${state.stagione}`);
  const vendite = res?.vendite || [];

  if (vendite.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5"><div class="empty-state"><div class="empty-icon">💰</div><p>Nessuna vendita registrata questa stagione</p></div></td></tr>';
    return;
  }

  tbody.innerHTML = vendite.map(v => `<tr>
    <td>${fmtDate(v.data_vendita)}</td>
    <td><span class="badge badge-${v.tipo === 'Olio' ? 'olio' : 'olive'}">${v.tipo}</span></td>
    <td class="text-right">${fmt(v.quantita)} ${v.tipo === 'Olio' ? 'L' : 'kg'}</td>
...
async function saveVendita(e) {
  ...
  if (data?.success) {
    showToast(`Vendita registrata! Ricavo: € ${fmt(data.ricavo)}`);
    form.reset();
...
function updateRicavo() {
  ...
  if (preview) {
    preview.textContent = q > 0 && p > 0 ? `= € ${fmt(q * p)} totale` : '';
  }
}
...
// ── STAGIONE SELECT ───────────────────────────
function changeStagione(val) {
  state.stagione = parseInt(val);
  navigate(state.page);
}

// ── INIT ──────────────────────────────────────
document.addEventListener('DOMContentLoaded', () => {
  // Imposta la data odierna nei form
  ...
  // Popola il select anno
  const stagSel = document.getElementById('stagione-select');
  if (stagSel) {
    const anno = new Date().getFullYear();
    for (let y = anno; y >= anno - 4; y--) {
      const opt = document.createElement('option');
      opt.value = y;
      opt.textContent = `Stagione ${y}`;
      if (y === anno) opt.selected = true;
      stagSel.appendChild(opt);
    }
    stagSel.addEventListener('change', () => changeStagione(stagSel.value));
  }
